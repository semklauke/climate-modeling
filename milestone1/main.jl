...
function read_geography()
    ...
    T = mapreduce(permutedims, vcat, vectors)
...
function robinson_projection(T::Matrix{Int8})
    ...
    for lat=1:65, long=1:128
        ...
        Y[lat, long] = (0.96047*𝜃 - 0.00857*sign(𝜃)*(abs(𝜃)^(6.41)))*-1.0
    end
    return X, Y
end
...
function plot_geo(X::Matrix, Y::Matrix, T::Matrix{Int8})
    ...
    for lat=1:64, long=1:127
        ...
        plot!(rect, fill=color_lookup[T[lat, long]], markerstrokewidth=0, linewidth=0)
    end
